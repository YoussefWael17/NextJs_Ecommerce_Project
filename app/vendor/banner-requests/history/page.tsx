...
export default function BannerRequestsHistoryPage() {
  ...
  function getInitialPage() {
    ...
    const saved = localStorage.getItem("banner_requests_page");
  ...
  const requests = data?.data?.bannerRequests ?? [];
  const pagination = data?.data?.pagination;

  const totalRequests = pagination?.total ?? 0;
  const pendingRequests = pagination?.pending ?? 0;
  const approvedRequests = pagination?.approved ?? 0;
  const rejectedRequests = pagination?.rejected ?? 0;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-black sm:text-3xl">
                Banner Requests History
              </h1>

              <p className="mt-2 text-sm text-gray-500">
                Track all your submitted banner requests and their status.
              </p>
            </div>
